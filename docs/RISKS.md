...
- **主场景**：Agent **回归测试与失败治理门禁** — 轨迹 → 失败分类 → 记录 → 发版前 `--compare` → CI golden
- 读 Format B JSON，用可验证启发式规则标记常见失败；本地 CLI，框架无关 adapter
...
- 需要本地 JSON 轨迹 + 失败分类 + **回归基线 / CI 门禁** 的中小型 Agent 团队  
- **不适合**当作生产 observability 唯一后端或合规审计唯一依据  

价值与业务证明进度：[VALUE.md](./VALUE.md)
